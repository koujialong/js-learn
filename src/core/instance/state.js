...
function initProps (vm: Component, propsOptions: Object) {
  ...
  for (const key in propsOptions) {
    ...
    // instantiation here.
    if (!(key in vm)) {
      proxy(vm, `_props`, key) // 将 _props 上的成员映射到 Vue 实例上, 是的不需要 app._props.xxx 来方法, 直接使用 app.xxx 来方法
    }
  }
...
export function getData (data: Function, vm: Component): any {
  ...
  pushTarget()  // 由于此时是 Vue 的初始化, 还没有进行模板渲染, 所以不需要进行依赖收集, 在 pushTarget 的时候传入 空
...
// 浏览器中触发的情况, 里面会对数据的访问关联一个 watcher
...
function initMethods (vm: Component, methods: Object) {
  ...
  for (const key in methods) {
    ...
    // 将 methods 属性中的方法 绑定上下文后 挂载到 vue 实例上
  }
}
